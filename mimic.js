...
function gameStart(canvas, img, face) {
	var resultEmojiCode = toUnicode(face.emojis.dominantEmoji);
	if(resultEmojiCode == code ) {
		setScore(10);
		alert("Correct");
		setTimeout(setTargetEmoji,3000);
	}
	else {
		setScore(Math.abs((resultEmojiCode - code)/4));
	}
}
// --- Callback functions ---
...
detector.addEventListener("onImageResultsSuccess", function(faces, image, timestamp) {
  ...
  if (faces.length > 0) {
    ...
    // <your code here>
	gameStart(canvas, image, faces[0]);
  }
});
...
function drawFeaturePoints(canvas, img, face) {
  ...
  for (var id in face.featurePoints) {
    ...
    // <your code here>
	ctx.beginPath();
	ctx.arc(featurePoint.x,featurePoint.y,2.5,0,2 * Math.PI);
	ctx.stroke();
  }
}
...
function drawEmoji(canvas, img, face) {
  ...
  // <your code here>
  var x=0,y=0,c=0;
	for(var i in face.featurePoints) {
		var featurePoint = face.featurePoints[i];
		x = Math.max(featurePoint.x);
		y = Math.max(featurePoint.y);
	}
  ctx.fillText(face.emojis.dominantEmoji,x+30,y);
}
